...
<div class="w-5/6 h-full flex justify-center items-center p-10 text-zinc-600">
    <ul class="w-full h-3/4 rounded-sm flex flex-col gap-y-1">
        <ul class="flex columns-8 gap-1">
            ...
            <a href="/admin/addUser" class="w-1/12 h-full items-center flex justify-center bg-gradient-to-bl from-orange-500 to-amber-400 hover:bg-gradient-to-tr text-2xl rounded-sm text-white">
            ...
            <ul class="flex columns-8 gap-1">
                ...
                <li class="w-1/3 border border-zinc-300">
                    <form id="changeRule_user${pd.id}" class="w-full h-full flex items-start justify-between text-start p-3">
                        <select name="setRule" class="hidden w-8/12 outline-none border focus:border-orange-500 rounded-sm p-2 px-3">
                        ...
                        <span name="saveRule" class="hidden button p-2 px-3 bg-gradient-to-tr from-lime-600 to-green-400 hover:bg-gradient-to-bl hover:-translate-y-1 active:translate-y-1 text-white rounded-sm"><i class="fa-solid fa-check"></i></span>
                        <span name="rule" class="button p-2 px-3">${pd.rule}</span>
                        <span name="change" class="${presentUser == pd.username ? 'hidden' : ''} button p-2 px-3 bg-gradient-to-tr from-zinc-600 to-gray-400 hover:bg-gradient-to-bl hover:-translate-y-1 active:translate-y-1 text-white rounded-sm"><i class="fa-solid fa-pen"></i></span>
                    </form>
                </li>
                <li class="w-1/12 border border-zinc-300 text-center flex py-3 items-${presentUser == pd.username ? 'center' : 'start'} justify-evenly">
                    ...
                    : `
                    <button id="delete_user${pd.id}" class="button p-2 px-3 bg-gradient-to-tr from-red-600 to-red-400 hover:bg-gradient-to-bl hover:-translate-y-1 active:translate-y-1 text-white rounded-sm"><i class="fa-regular fa-trash-can"></i></button>
                    `}
                </li>
            </ul>
            `).join('')}
